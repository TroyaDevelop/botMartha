...
def calculate_roll(command: str) -> str:
    ...
    for cmd in commands:
        ...
        if split_index == -1:
            message += f"Некорректная команда. Убедитесь в правильности формата, например: /д20+5.\n"
            continue
        ...
        if not dice_sides_str.isdigit():
            message += f"Некорректное количество граней в команде.\n"
            continue
        ...
        if dice_count <= 0 or dice_sides <= 0:
            message += f"Количество кубиков и граней должно быть положительным числом в команде.\n"
            continue
